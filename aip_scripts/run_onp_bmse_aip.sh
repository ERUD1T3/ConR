...
#SBATCH --job-name=onp_bmse_aip       # Job name
...
echo "Starting ONP BMSE training at date $(date)"
...
# --- Configuration based on run_onp_bmse_chan.sh ---
...
if [ $? -eq 0 ]; then
    echo "Successfully completed ONP BMSE training"
else
    echo "Error in ONP BMSE training"
    exit 1
fi
...
# Usage:
# sbatch aip_scripts/run_onp_bmse_aip.sh 