...
#SBATCH --job-name=bf_bmse_aip        # Job name
...
echo "Starting BF BMSE training at date $(date)"
...
# --- Configuration based on run_bf_bmse_chan.sh ---
...
# BMSE / GAI specific settings - Updated for BF dataset with appropriate K value
GMM_FILE="/home1/jmoukpe2016/BalancedMSE/neurips2025/gmm/bf_gmm_K16.pkl"
DATA_DIR="/home1/jmoukpe2016/BalancedMSE/neurips2025/data"

# Lower and upper thresholds for label range categorization (BF specific)
...
if [ $? -eq 0 ]; then
    echo "Successfully completed BF BMSE training"
else
    echo "Error in BF BMSE training"
    exit 1
fi
...
# Usage:
# sbatch aip_scripts/run_bf_bmse_aip.sh 